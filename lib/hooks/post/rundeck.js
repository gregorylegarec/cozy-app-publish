...
const runRundeckJob = (token, instance, slug) =>
  ...
  const request = https.request({
    ...
    method: 'POST',
    path: encodeURI(`/api/20/job/${job}/run?argString=-instance+${instance}+-slugs+${slug}`)
  }, res => {
    if (res.statusCode === 200) {
      resolve(res)
    }
    else {
      reject(new Error('Rundeck response error'))
    }
  })
...
module.exports = async options => {
  ...
  console.log(`↳ ℹ️  Updating channel ${channel} in instances ${targets.replace(',', ', ')}`)

  const rundeckJob = RUNDECK_JOBS[channel]
