...
const program = new commander.Command(pkg.name)
  .version(pkg.version)
  .usage(`[options]`)
  .option('--editor', 'the registry editor to publish as (required)')
  .option('--token', 'the registry token matching the provided editor (required)')
  .option('--build-dir', 'path fo the build directory relative to the current directory (default ./build)')
  .option('--build-url', 'URL of the application archive')
  .option('--manual-version', 'publishing a specific version manually (must not be already published in the registry)')
  .option('--on-branch', 'the branch name to run this script on (default: build)')
  .option('--registry-url', 'the registry URL to publish to a different one from the default URL')
